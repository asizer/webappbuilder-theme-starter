...
To allow for css preprocessing, this starter repo adds a "preprocessing" folder -- a place to put all your sass files. Grunt then watches this folder for changes, runs sass on any top-level non-partial sass files within the folder, and outputs the results to a style.css file within a folder named for the source sass file.
...
V preprocessing
  _overrides.scss  // because this is a partial (starts with an underscore), it won't be output anywhere
  _panels.scss     // not processed (partial)
  common.scss      // this file will be processed and output to [themeroot]/common.css
  default.scss     // to styles/default/style.css
  fuchsia.scss     // to styles/fuchsia/style.css
  > jimu-overrides // nothing in this folder will be processed
  lime.scss        // to styles/lime/style.css
